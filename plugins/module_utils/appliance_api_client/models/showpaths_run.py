...
class ShowpathsRun(BaseModel):
    ...
    no_probe: Optional[StrictBool] = Field(False, description="Do not probe the paths.")
    refresh: Optional[StrictBool] = Field(False, description="Set refresh flag for the path request.")
    maxpaths: Optional[conint(strict=True, ge=0)] = Field(0, description="Maximum number of paths that are returned.")
    ...
    timeout: Optional[StrictStr] = Field('1s', description="Timeout for the showpaths request. Valid time units are \"ms\", \"s\".")
    __properties = ["destination_isd_as", "source_isd_as", "no_probe", "refresh", "maxpaths", "sequence", "timeout"]
    ...
    @classmethod
    def from_json(cls, json_str: str) -> ShowpathsRun:
    ...
    @classmethod
    def from_dict(cls, obj: dict) -> ShowpathsRun:
        """Create an instance of ShowpathsRun from a dict"""
        if obj is None:
            return None

        if not isinstance(obj, dict):
            return ShowpathsRun.parse_obj(obj)

        _obj = ShowpathsRun.parse_obj({
