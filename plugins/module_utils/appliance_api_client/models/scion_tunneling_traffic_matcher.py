...
class ScionTunnelingTrafficMatcher(BaseModel):
    ...
    failover_sequence: conlist(ScionTunnelingFailoverSequenceEntry) = Field(..., description="A failover sequence. If there are no healthy sessions in the first entry, the second one is used and so on. ")
    ...
    __properties = ["traffic_policy_id", "traffic_matcher", "failover_sequence", "selected_session", "selected_session_legacy", "selected_path"]
    ...
    @classmethod
    def from_json(cls, json_str: str) -> ScionTunnelingTrafficMatcher:
    ...
    @classmethod
    def from_dict(cls, obj: dict) -> ScionTunnelingTrafficMatcher:
        """Create an instance of ScionTunnelingTrafficMatcher from a dict"""
        if obj is None:
            return None

        if not isinstance(obj, dict):
            return ScionTunnelingTrafficMatcher.parse_obj(obj)

        _obj = ScionTunnelingTrafficMatcher.parse_obj({
