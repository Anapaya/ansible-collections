# coding: utf-8

"""
    Appliance Management API

    Management API for the Anapaya EDGE, CORE and GATE appliances

    The version of the OpenAPI document: 0.1.0
    Contact: ops@anapaya.net
    Generated by OpenAPI Generator (https://openapi-generator.tech)

    Do not edit the class manually.
"""  # noqa: E501


import unittest
import datetime

from openapi_client.models.ping_summary import PingSummary

class TestPingSummary(unittest.TestCase):
    """PingSummary unit test stubs"""

    def setUp(self):
        pass

    def tearDown(self):
        pass

    def make_instance(self, include_optional) -> PingSummary:
        """Test PingSummary
            include_option is a boolean, when False only required
            params are included, when True both required and
            optional params are included """
        # uncomment below to create an instance of `PingSummary`
        """
        model = PingSummary()
        if include_optional:
            return PingSummary(
                path = openapi_client.models.path.Path(
                    fingerprint = 'f', 
                    hops = [
                        openapi_client.models.path_segment_hop.Path segment hop(
                            isd_as = '1-ff00:0:110', 
                            interface = 42, )
                        ], 
                    next_hop = '', 
                    expiry = datetime.datetime.strptime('2013-10-20 19:20:30.00', '%Y-%m-%d %H:%M:%S.%f'), 
                    mtu = 0, 
                    latency = [
                        ''
                        ], 
                    status = 'unknown', 
                    local_ip = '', ),
                payload_size = 56,
                scion_packet_size = 56,
                replies = [
                    openapi_client.models.ping_update.PingUpdate(
                        scion_packet_size = 56, 
                        source_isd_as = '1-ff00:0:110', 
                        source_host = '', 
                        scmp_seq = 56, 
                        round_trip_time = '', 
                        state = 'success', )
                    ],
                statistics = openapi_client.models.ping_statistics.PingStatistics(
                    sent = 56, 
                    received = 56, 
                    packet_loss = 1.337, 
                    time = 56, )
            )
        else:
            return PingSummary(
                path = openapi_client.models.path.Path(
                    fingerprint = 'f', 
                    hops = [
                        openapi_client.models.path_segment_hop.Path segment hop(
                            isd_as = '1-ff00:0:110', 
                            interface = 42, )
                        ], 
                    next_hop = '', 
                    expiry = datetime.datetime.strptime('2013-10-20 19:20:30.00', '%Y-%m-%d %H:%M:%S.%f'), 
                    mtu = 0, 
                    latency = [
                        ''
                        ], 
                    status = 'unknown', 
                    local_ip = '', ),
                payload_size = 56,
                scion_packet_size = 56,
                replies = [
                    openapi_client.models.ping_update.PingUpdate(
                        scion_packet_size = 56, 
                        source_isd_as = '1-ff00:0:110', 
                        source_host = '', 
                        scmp_seq = 56, 
                        round_trip_time = '', 
                        state = 'success', )
                    ],
                statistics = openapi_client.models.ping_statistics.PingStatistics(
                    sent = 56, 
                    received = 56, 
                    packet_loss = 1.337, 
                    time = 56, ),
        )
        """

    def testPingSummary(self):
        """Test PingSummary"""
        # inst_req_only = self.make_instance(include_optional=False)
        # inst_req_and_optional = self.make_instance(include_optional=True)

if __name__ == '__main__':
    unittest.main()
