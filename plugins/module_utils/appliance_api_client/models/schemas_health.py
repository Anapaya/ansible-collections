...
class SchemasHealth(BaseModel):
    """
    SchemasHealth
    """
    status: HealthStatus = Field(...)
    checks: conlist(HealthCheck) = Field(...)
    __properties = ["status", "checks"]
    ...
    @classmethod
    def from_json(cls, json_str: str) -> SchemasHealth:
    ...
    @classmethod
    def from_dict(cls, obj: dict) -> SchemasHealth:
        """Create an instance of SchemasHealth from a dict"""
        if obj is None:
            return None

        if not isinstance(obj, dict):
            return SchemasHealth.parse_obj(obj)

        _obj = SchemasHealth.parse_obj({
            "status": obj.get("status"),
            "checks": [HealthCheck.from_dict(_item) for _item in obj.get("checks")] if obj.get("checks") is not None else None
        })
        return _obj
