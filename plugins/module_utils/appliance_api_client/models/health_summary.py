...
class HealthSummary(BaseModel):
    ...
    """
    status: Status = Field(...)
    checks: conlist(CheckSummary) = Field(..., description="List of health checks.")
    ...
    @classmethod
    def from_dict(cls, obj: dict) -> HealthSummary:
        """Create an instance of HealthSummary from a dict"""
        if obj is None:
            return None

        if not isinstance(obj, dict):
            return HealthSummary.parse_obj(obj)

        _obj = HealthSummary.parse_obj({
            "status": obj.get("status"),
            "checks": [CheckSummary.from_dict(_item) for _item in obj.get("checks")] if obj.get("checks") is not None else None
        })
        return _obj
