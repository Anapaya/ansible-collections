...
class PingRun(BaseModel):
    ...
    count: Optional[Annotated[int, Field(le=65535, strict=True, ge=1)]] = Field(default=1, description="When the parameter is set, ping sends the specified number of SCMP echo packets and reports back the statistics. If this is not set, a single SCMP echo packet is sent. ")
    ...
    max_mtu: Optional[StrictBool] = Field(default=None, description="Choose the payload size such that the sent SCION packet including the SCION Header, SCMP echo header and payload are equal to the MTU of the path. This flag overrides the 'payload_size' flag. ")
    payload_size: Optional[Annotated[int, Field(strict=True, ge=8)]] = Field(default=8, description="Number of bytes to be sent in addition to the SCION Header and SCMP echo header. The total size of the packet is still variable size due to the variable size of the SCION path. ")
    ...
    __properties: ClassVar[List[str]] = ["destination_host", "destination_isd_as", "count", "source_isd_as", "max_mtu", "payload_size", "sequence", "timeout"]
