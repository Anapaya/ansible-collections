...
class AnnounceFilterMatchesPrefixData(BaseModel):
    """
    The entry in the announce filter matches at least one prefix that is announced in a domain. While it is not invalid to have an announce filter entry that does not match anything, it is certainly curious. This health check is meant to help operators identify such cases. This only applies to the entries with action `accept`.  If `count` is zero, the status is `notice`. Otherwise, the status is `passing`.   # noqa: E501
    """
    domain: StrictStr = Field(..., description="The domain the announce filter entry is part of. ")
    ...
    @classmethod
    def from_json(cls, json_str: str) -> AnnounceFilterMatchesPrefixData:
    ...
    @classmethod
    def from_dict(cls, obj: dict) -> AnnounceFilterMatchesPrefixData:
        """Create an instance of AnnounceFilterMatchesPrefixData from a dict"""
        if obj is None:
            return None

        if not isinstance(obj, dict):
            return AnnounceFilterMatchesPrefixData.parse_obj(obj)

        _obj = AnnounceFilterMatchesPrefixData.parse_obj({
