# coding: utf-8

"""
    Appliance Management API

    Management API for the Anapaya EDGE, CORE and GATE appliances

    The version of the OpenAPI document: 0.1.0
    Contact: ops@anapaya.net
    Generated by OpenAPI Generator (https://openapi-generator.tech)

    Do not edit the class manually.
"""  # noqa: E501


import unittest
import datetime

from openapi_client.models.external_interface_up import ExternalInterfaceUp

class TestExternalInterfaceUp(unittest.TestCase):
    """ExternalInterfaceUp unit test stubs"""

    def setUp(self):
        pass

    def tearDown(self):
        pass

    def make_instance(self, include_optional) -> ExternalInterfaceUp:
        """Test ExternalInterfaceUp
            include_option is a boolean, when False only required
            params are included, when True both required and
            optional params are included """
        # uncomment below to create an instance of `ExternalInterfaceUp`
        """
        model = ExternalInterfaceUp()
        if include_optional:
            return ExternalInterfaceUp(
                data = openapi_client.models.external_interface_up_data.ExternalInterfaceUpData(
                    local_isd_as = '1-ff00:0:110', 
                    local_address = '', 
                    local_interface = 56, 
                    remote_isd_as = '1-ff00:0:110', 
                    remote_address = '', 
                    remote_interface = 56, 
                    relationship = 'CHILD', 
                    state = 'UP', 
                    bfd_enabled = True, )
            )
        else:
            return ExternalInterfaceUp(
                data = openapi_client.models.external_interface_up_data.ExternalInterfaceUpData(
                    local_isd_as = '1-ff00:0:110', 
                    local_address = '', 
                    local_interface = 56, 
                    remote_isd_as = '1-ff00:0:110', 
                    remote_address = '', 
                    remote_interface = 56, 
                    relationship = 'CHILD', 
                    state = 'UP', 
                    bfd_enabled = True, ),
        )
        """

    def testExternalInterfaceUp(self):
        """Test ExternalInterfaceUp"""
        # inst_req_only = self.make_instance(include_optional=False)
        # inst_req_and_optional = self.make_instance(include_optional=True)

if __name__ == '__main__':
    unittest.main()
